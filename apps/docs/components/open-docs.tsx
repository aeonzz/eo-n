...
function getPromptUrl(baseURL: string, url: string) {
  return `${baseURL}?q=${encodeURIComponent(
    `I’m looking at this eo-n/ui documentation: ${url}.
...
const menuItems = {
  v0: (url: string) => (
    ...
    >
      ...
      <span key="label" className="-translate-x-[2px]">
        Open in v0
      </span>
    </a>
  ),
  chatgpt: (url: string) => (
    <a
      ...
      rel="noopener noreferrer"
    >
      <svg xmlns="http://www.w3.org/2000/svg" key="icon" viewBox="0 0 24 24">
        <path
          key="path1"
      ...
      <DropdownMenuTrigger
        render={
          <Button
            variant="secondary"
            size="sm"
            className="h-8 md:h-7 md:text-[0.8rem] [&_svg]:size-4"
          >
            Open
